    ...
    for (let i = 0; i < tools.length; i++) {
      toolsWithAds.push(
        <ToolCard
          key={tools[i].id}
          title={tools[i].title}
          category={tools[i].aiToolCategories.nodes[0]?.name || "AI Tool"}
          slug={tools[i].slug}
          previewImage={tools[i].featuredImage?.node?.sourceUrl || "/placeholder.svg"}
          logo={tools[i].featuredImage?.node?.sourceUrl || "/placeholder.svg"}
          isVerified={Math.random() > 0.5}
        />
      );

      // Insert AdSense after approximately every 8 tools
      if ((i + 1) % 8 === 0 && i !== tools.length - 1) {
        toolsWithAds.push(
          <div key={`ad-${i}`} className="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4">
