...
async function getCategoryTools(category: string, first: number = 20, after: string | null = null): Promise<AIToolsResponse> {
  ...
  if (!res.ok) {
    throw new Error(`Failed to fetch Category Tools: ${res.status} ${res.statusText}`);
  }
  return res.json();
}
...
export function CategoryToolsSection({ category, categoryName }: CategoryToolsSectionProps) {
  ...
  const loadTools = async (isInitial: boolean = false) => {
    setLoading(true);
    try {
      const data = await getCategoryTools(category, 20, isInitial ? null : endCursor);
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <h2 className="text-3xl font-bold text-white mb-8">{categoryName} AI Tools</h2>
